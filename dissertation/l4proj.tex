...
We also implement the accessory plotting tag and spectra-plotting tool we mentioned in the requirements earlier - an illustration can be seen in Figure \ref{fig:spectragraph}.
...
This performance is less than ideal, so we would like to improve it. The most glaring bottleneck is the factorial-time (with tag-length) algorithm used for alignment; although it usually operates on very small data, this is an inefficient enough algorithm to hugely hurt performance even on small data, and to make solutions impossible for larger data. This is the number one area requiring attention in any potential future development; see Section \ref{nrpalg} for more information.
